...
export function WorkloadTable({ workloads }: WorkloadTableProps) {
  ...
  const [scaleOpen, setScaleOpen] = useState<{ ns: string; name: string; current: number } | null>(null);
  const [desired, setDesired] = useState<number>(0);

  useEffect(() => {
    if (scaleOpen) setDesired(scaleOpen.current);
  }, [scaleOpen]);
  ...
  const filtered = useMemo(() => {
    return workloads.filter(w => (namespaceFilter === "all" || w.namespace === namespaceFilter) && w.name.toLowerCase().includes(query.toLowerCase()));
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      className="rounded-md border border-white/10 px-2 py-1 text-xs text-white/80 hover:bg-white/10"
                      onClick={async () => {
                        const clusterId = typeof window === "undefined" ? undefined : localStorage.getItem("clusterId") ?? undefined;
                        await apiFetch(`/clusters/${clusterId}/namespaces/${workload.namespace}/deployments/${workload.name}/pause`, { method: "POST" });
                        alert("Rollout paused (202)");
                      }}
                    >
                      Pause
                    </button>
                    <button
                      ...
                      onClick={async () => {
                        const clusterId = typeof window === "undefined" ? undefined : localStorage.getItem("clusterId") ?? undefined;
                        await apiFetch(`/clusters/${clusterId}/namespaces/${workload.namespace}/deployments/${workload.name}/resume`, { method: "POST" });
                        alert("Rollout resumed (202)");
                      }}
                    >
                      Resume
                    </button>
                    <button
                      ...
                      onClick={async () => {
                        const clusterId = typeof window === "undefined" ? undefined : localStorage.getItem("clusterId") ?? undefined;
                        await apiFetch(`/clusters/${clusterId}/namespaces/${workload.namespace}/deployments/${workload.name}/restart`, { method: "POST" });
                        alert("Restart triggered (202)");
                      }}
                    >
                      Restart
                    </button>
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-background p-6 shadow-2xl">
            ...
            <div className="mt-4 space-y-3">
              ...
              <input
                ...
                className="w-full"
              />
              <div className="text-xs text-white/60">Current: {scaleOpen.current} → New: {desired}</div>
            </div>
            <div className="mt-6 flex justify-end gap-3">
              <button className="rounded-md border border-white/10 px-4 py-2 text-sm text-white/80 hover:bg-white/10" onClick={() => setScaleOpen(null)}>Annuleren</button>
              <button
                className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-black"
                onClick={async () => {
                  const clusterId = typeof window === "undefined" ? undefined : localStorage.getItem("clusterId") ?? undefined;
                  await apiFetch(`/clusters/${clusterId}/namespaces/${scaleOpen.ns}/deployments/${scaleOpen.name}/scale`, { method: "POST", body: JSON.stringify({ replicas: desired }) });
                  setScaleOpen(null);
                }}
              >
                Scale to {desired}
